...
- `config/demos.yml`: `deep-research` の定義。一覧とデモの画面の表示が「準備中」から、Vertex AI の設定値の有無に応じて「実行できる」または「設定値が足りない（VertexAI）」に変わる（プロバイダー名は RubyLLM の `display_name` で、空白を含まない）
...
| AC-47 | デモ定義 | 異常・拒否 | Vertex AI の設定値がないとき、一覧とデモの画面で「設定値が足りない（VertexAI）」（RubyLLM の `display_name`）になり、実行ボタンが無効になり、実行を指示しても記録されない | テスト |
...
| AC-62 | ジョブ | 状態・権限 | 投入が続けられる種類の例外（レート制限など）で失敗したとき（ID を控える前）は、やり直さず、その種類で失敗として記録し、ジョブは投入されない | テスト |
| AC-63 | ジョブ | 状態・権限 | ID を控えていない実行で 404 の例外が起きたときは、「期限切れ」ではなく表の種類で失敗として記録する | テスト |
| AC-64 | ジョブ | 状態・権限 | プロバイダー側の取り消しの例外は、ID を控えているかによらず取り消しとして記録する | テスト |
| AC-65 | 処理 | 異常・拒否 | 再開の読み込み（`ResearchJob.find`）の例外は、包まずにそのまま伝わる | テスト（差し替え） |
| AC-66 | 処理 | 正常 | プロバイダーがコストを返したときは、結果のコストがその数値になる | テスト（差し替え） |
| AC-67 | 失敗の種類 | 境界 | 表にない例外を `cause` に持つ `TimeoutError`（締め切りで中断された状態の取得）は「待ち時間の上限」を返し、続けられる判定は偽になる | テスト |

AC-62〜AC-67 は、実装後の照合で見つかった、実装にあって表になかった条件である（利用者の判断で追加）。同じ照合で見つかった次の条件は、利用者の判断で受け入れ条件に加えず許容した。

- 開始が空の ID の処理を返すと、控えずにアプリの不具合として報告される。RubyLLM は空の ID のジョブを返さないため、返れば不具合として扱うのが正しい
- `id` と `pending?` の両方を持つ値だけをプロバイダーに残した処理として扱う。AC-20 の前提そのもの
- 表示の防御的な分岐（タイトルのない出典を URL でリンクする、手順の詳細が文字列ならそのまま出し両方なければ出さない、結果の項目がなくても空として描画する）
- ワーカーの異常終了によるやり直しの表示にはプロバイダー名が出ない。異常終了はプロバイダーの呼び出しではなく、既存の `fail_as!` の失敗と同じ扱い
