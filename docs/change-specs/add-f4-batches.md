...
- OpenAI は、24 時間以内に終わらなかったバッチを `expired` にし、未処理の要求を取り消す。完了した要求の回答は結果ファイルに残り、課金される。取り消したバッチも、処理済みの要求の回答は出力ファイルに残る（OpenAI の API リファレンス）。取り消したバッチの完了した分が課金されるかは、OpenAI の文書に書かれていない
...
ジョブが実行を受け取ったときの分岐は次のとおりである。控えのある実行の確認は workflow の外で行い、回収だけを workflow の内側で行う。回収は、バッチが終端に達したときに、その状態によらず行う。期限切れと取り消しでも完了した分の回答は残り、期限切れでは課金もされているためである。
...
- **追加**: F4 の代表シナリオの処理。チケットの本文とモデルを受け取る
  ...
  - 回収（クラスメソッド。バッチ ID を受け取る）。通常の文脈でバッチを復元し、`messages` で回答を回収して各会話の記録に追加し、その後に `statuses` を読んで結果を組み立てる。復元した会話の利用者のメッセージからチケットの本文を、回答の本文を JSON として読んで区分と理由を得る（スキーマは記録に残らないが、回答は構造化出力なので読める）。JSON として読めない回答（拒否の文面など）と、区分のない回答は、成否を `succeeded` のまま区分と理由を `nil` にし、バッチの他のチケットの結果を残す（実行全体を失敗にすると、課金された他の回答の結果が残らないため。利用者の決定）。使用量のイベントは通常の文脈で発行される
...
- **追加**: 失敗の種類の表に、表 `TABLE` の外の 2 つを加える。プロバイダー側の処理の失敗（原因の候補: 期限切れ（expired）なら 24 時間以内に処理されなかった。完了した分は課金され、結果に残る。OpenAI のダッシュボードでバッチの状態と理由を確かめる）と、プロバイダー側の処理の取り消し（原因の候補: OpenAI のダッシュボードなどでバッチが取り消された。取り消す前に完了した分は結果に残る）である。`for` と `provider_call?` の判定は変えない
- **変更**: 実行のジョブ
...
- **追加**: F4 の結果の表示部品（`result_kind` は `batch_classification`）。全体として、OpenAI の状態文字列、件数の内訳（成功 M 件、失敗 K 件、全 N 件）、バッチの ID、応答したモデルを表示する。チケットごとに、本文、成否のバッジ（成功、失敗、取り消し）、区分、理由を投入順に表示する。回答のないチケットは区分と理由の代わりに、失敗したチケットには失敗の理由が RubyLLM のログ（このアプリでは標準出力）に出る旨を、取り消されたチケットには取り消されたため処理されなかった旨を表示する。成功したのに区分のないチケットには、回答はあるが区分として読めなかった旨と、回答の本文が会話の記録にある旨を表示する（利用者の決定）
- **変更**: 実行の画面の状態で変わる部分
...
| 6 | 終端に達したバッチのうち、期限切れと取り消しを回収するか（回収しない、成功と同じく回収して結果つきで失敗か取り消しにする） | 回収する。OpenAI は完了した要求の回答を残し、期限切れでは課金する。回収しないと、課金された回答と、チケットごとの成否（2.5）と、そのコスト（F4）が残らない | なし |
...
| AC-4 | 処理（投入） | 状態・権限 | 開始済みの実行でジョブが再び動いても、控えがあれば投入をやり直さない（AC-25）。控えも会話の記録もなければ「ジョブの中断」で失敗になり、投入しない | テスト |
...
| AC-31 | 表示（結果と理由） | 異常・拒否 | 回答のない失敗したチケットは、区分と理由の代わりに、回答がない旨と、理由が RubyLLM のログに出る旨が表示される。回答のない取り消されたチケットは、取り消されたため処理されなかった旨が表示される | テスト |
...
| AC-46 | 処理（確認・回収） | 異常・拒否 | JSON として読めない回答と、区分のない回答のチケットは、成否が `succeeded` のまま区分（と読めない場合は理由）が `nil` になり、他のチケットの結果は残る。回答は会話の記録に残る（実装後の照合で追加） | テスト |
| AC-47 | 表示（結果と理由） | 異常・拒否 | 成功したのに区分のないチケットは、回答はあるが区分として読めなかった旨と、本文が会話の記録にある旨が表示され、回答がない旨は表示されない（実装後の照合で追加） | テスト |
| AC-48 | 処理（確認・回収）、表示（結果と理由） | 境界 | 全チケットが失敗した結果は、応答したモデルが `nil` で、画面では「—」になる（実装後の照合で追加） | テスト |
| AC-49 | ジョブ | 境界 | 投入から 48 時間を過ぎた後の回収でプロバイダーの呼び出しに由来する例外が起きると、確認と同じく、その例外の種類で失敗になり、次の確認を予約しない（実装後の照合で追加） | テスト |
| AC-50 | 実行の記録（控え、結果つきの失敗と取り消し、トレースの日時） | 異常・拒否 | 表にない例外による確認の失敗は、例外のクラス名を種類として記録する（実装後の照合で追加） | テスト |
| AC-51 | 購読者 | 境界 | workflow も操作も囲まない `usage.ruby_llm` も、その操作のスパン（独立したトレース）になる。操作が既知の操作でなければ attempt のスパンのままになる。コストの合計が `nil` で、金額のない部分が出力に含まれる推論トークンだけのときは、部分の金額の和がコストとして載る。他の部分の金額がないか、推論トークンが出力トークンを超えるときは、コストは載らない（実装後の照合と実 API で追加） | テスト |

### 許容した条件

実装後の照合で見つかった、受け入れ条件にない条件のうち、次の 11 件は利用者の決定で許容する。

- 投入に失敗しても、先に作った会話の記録は残る。記録は履歴として無害で、投入の失敗は実行の失敗として画面に出る
- 保存のないバッチ ID を回収すると `ArgumentError` になり、アプリの不具合として失敗と報告になる。ジョブは保存した ID しか渡さない
- `Batch` 以外の `id` と `status` を持つ値（`ResearchJob`、`VideoJob`）は `ArgumentError` になる。F6b か F8 の実装で種類ごとの読み方を足す
- 会話の記録と控えの両方を持つ実行は、会話の記録の経路で続く。両方を持つ代表シナリオはない
- 承認待ちの実行への確認の結果と確認の失敗の記録は拒否される。控えの記録と同じ判定で、ジョブからは到達しない
- 控えのない実行への確認の結果と確認の失敗の記録は `NoMethodError` になる。ジョブは控えのある実行でしか呼ばない
- 文字列だけの既存のトレース ID と同じ ID は、もう一度加えない。既存どおりの重複の排除である
- 失敗の内容のない取り消しの実行では、理由の区画を出さない。そうした実行を作る経路はない
- 知らない成否の値は、その文字列のまま取り消しの色で表示する。RubyLLM の成否は 3 つだけである
- 件数の内訳が空のハッシュでも「—」になり、キーが欠けていれば 0 件と表示する。OpenAI は 3 つのキーを常に返す
- 結果のない取り消しを記録できる。取り消しの記録の既定である
