...
| AC-20 | 実行の承認待ちの記録 | 境界 | 決定待ちの呼び出しが 0 件の会話の記録で承認待ちにしようとすると例外になり、実行は実行中のままで、会話の記録の ID は記録されない（決定できない承認待ちを作らない） | テスト |
...
| AC-48 | 決定の受付 | 状態・権限 | 代表シナリオの定義が失われた承認待ちの実行に決定を送ると、続きを実行できない旨を添えた実行の画面が 422 で返り、何も記録されず、ジョブは投入されない | テスト |
| AC-49 | 実行の承認待ちの記録 | 状態・権限 | ワーカーの異常終了で失われたジョブの実行を失敗にする既存の操作は、承認待ちの実行を変えない | テスト |
| AC-50 | 処理 | 境界 | 会話に `issue_refund` の呼び出しが複数あるとき、結果の注文は最初の呼び出しの引数から求める | テスト |

AC-48〜AC-50 は、実装後の照合で見つかった、この表になかった条件である。AC-20 は「該当なし」としていたが、実行の側に歯止めがなかったため、条件として書き直した。

#### 実装後の照合で許容した条件

- 注文の説明が空白なら作れず、状態は支払い済みか返金済みのどちらかである: モデルの検証で、処理は必須の入力から説明を渡すため、画面では起きない
- 会話の記録が失われた実行を再開しようとすると、記録が見つからない例外になる: 実行から会話の記録への外部キーがあり、起きない。起きれば既存の扱いどおり、失敗として記録し、アプリの不具合として報告する
- 決定と注文の状態の表示は、知らない値をそのまま表示する: 表示の取りこぼしを防ぐための扱いで、処理が返す値は決まっている
